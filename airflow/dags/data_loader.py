...
def extract_data(ti):
    ...
    data = dvc.api.get_url(path=data_url,repo=repo,rev=version)
    loaded_df_name=data_extractor.extract_data(file_name=data,return_json=True)
...
def populate_table(ti):
    ...
    util.insert_to_table(trajectory_file_name, 'trajectories',from_file=True)
    util.insert_to_table(vehicle_file_name, 'vehicles',from_file=True)
